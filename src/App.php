    ...
    public function bootstrap()
    {
        ...
        if (!is_null($this->controller)) {
            ...
            if ($lti_version === LTIConstants::LTI_VERSION_1) {
                ...
                $course_id =  $LTI->ltiRawParameter(LTIConstants::CONTEXT_ID, false);
            ...
            $independent_activity_id = ParamValidate::independentActivityInCustom($_SESSION) ?: ParamValidate::independentActivityInQueryString($_SESSION);
            ...
            if ($project_id) {
                ...
                header("Location: $redirect_to_studio_url");
            } elseif ($independent_activity_id && $is_summary != 1) {
                ...
                if ($isMoodleSummaryPage) {
                    ...
                    if (!U::get($_GET, "is_submission_review")) {
                        ...
                        $student_result = $PDOX->allRowsDie($student_result);
                        $result_id_x = $student_result[0]['result_id'];
                        $is_submission_review = base64_encode("result_id={$result_id_x}&independent_activity_id={$independent_activity_id}&user_id={$student_pk_id}");
                ...
                if ($is_gradepassback) {
                    ...
                    $review_data = [];
                    $review_data['result_id'] = $lti_data['result_id'];
                    $review_data['independent_activity_id'] = $independent_activity_id;
                    $review_data['user_id'] = $user_id;
                ...
                $submission_id = $LTI->result->id;
                $lti_token_params = http_build_query($_SESSION['lti_post']);
                $activity_studio_link = CURRIKI_STUDIO_HOST . "/lti-tools/independent_activity/$independent_activity_id";
                $redirect_to_studio_url = $activity_studio_link . "?" . $lti_token_params;
                $custom_variable_array = array(
                    'user_id',
                    'tool_platform',
